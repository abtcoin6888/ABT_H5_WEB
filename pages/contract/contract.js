...
export default {
    ...
    methods: {
        ...
        onContractMessageHandler(data) {
            //接收ws 的数据
            if ((data.type === 200 && cm_id === data.data.cm_id) || (data.type === 500 && data.data.cm_id === cm_id)) {

                try {
                    let ticker = data.data.data[this.currentCurrency['id'].toString()];
            ...
            if(data.type === 200 || data.type === 500){
                ...
                this.currency = this.currency.map((item)=>{
                    if(item.id === data.data.cm_id){
                        item['currency'] = this.$util.compareCurrency(item.currency,data.data.data);
                    }
                    return item;
                });
            }

            if ((data.type === 201 || data.type === 501) && data.data.currency_id === this.currentCurrency['id']) {
                let length = 9;
                if (!this.isLogin) {
                    length = 3;
                } else {

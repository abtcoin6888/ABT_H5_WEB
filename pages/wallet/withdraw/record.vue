<template>
    <view class="record" :data-theme="userTheme">
        ...
        <fui-nav-bar :title="$t('wrcord.t1')"
                     ...
                     isFixed
                     isOccupy
                     @leftClick="$mroute.back()" @init="navInit">
        ...
        <scroll-view class="scroll-view"
            ...
            <view class="list">
                ...
                <view class="cell" v-for="(item, index) in list" v-if="list.length">
                    ...
                    <view class="coin-info">
                        ...
                        <view class="row">
                            ...
                            <text class="font-12 color-gray" v-if="item.status === 0">{{ $t('wrcord.t2') }}</text>
                            <text class="font-12 color-gray" v-if="item.status === 1">{{ $t('wrcord.t3') }}</text>
                            <text class="font-12 color-gray" v-if="item.status === 2">{{ $t('wrcord.t4') }}</text>
                        </view>
                    </view>
                </view>
                <view class="not-date" :style="{'height': (windowHeight / 2) + 'px'}" v-else>
                    <fui-empty descrSize="26"
                               descrColor="var(--content-tertiary)"
                               width="200"
                               height="200"
        ...
        <!-- 时间 -->
        <fui-date-picker range :theme="userTheme"
                         rangeBackground="var(--color-primary)"
                         :title="$t('recod.t4')"
						 :start="$t('开始时间')"
						 :end="$t('结束时间')"
                         titleSize="30"
        ...
        <fui-bottom-popup background="var(--background-primary)" radius="30" :show="coinTypeShow" @close="closePopup">
            <view class="popup-wrap" type="coinList">
                ...
                <view class="search">
                    <fui-icon name="search" size="32" color="var(--color-text-dark-gray)"></fui-icon>
                ...
                <scroll-view scroll-y :show-scrollbar="false" class="list">
                    <view class="cell" :class="coinIndex == index ? 'cell-active' : ''" v-for="(item,index) in coinList"
                        ...
                        <text class="font-weight" v-if="item">{{ item.symbol }}</text>
                    </view>
                </scroll-view>
            </view>
        </fui-bottom-popup>
...
export default {
    ...
    methods: {
        ...
        async getRechargeLog() {
            ...
            let res = await currency.getWithLog(this.param)
